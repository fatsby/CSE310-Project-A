...
function calcSummary(items) {
  ...
  const totalPurchases = items.reduce((s, it) => s + (it.purchaseCount || 0), 0);
  return { uploads, avgRating, totalPurchases };
}

function ItemCard({ item }) {
  return (
    <div className="rounded-2xl bg-white shadow-md border p-4 flex flex-col">
  ...
  const filtered = useMemo(() => {
    ...
    switch (sort) {
      ...
      case "ratingDesc":
        ...
        break;
      case "purchasedDesc":
        res.sort((a, b) => (b.purchaseCount || 0) - (a.purchaseCount || 0));
        break;
      case "recent":
      default:
        res.sort((a, b) => parseDMY(b.lastUpdated) - parseDMY(a.lastUpdated));
        break;
    }
    return res;
    ...
    <div className="container mx-auto px-4 pb-4 pt-[125px]">
        ...
        <div className="lg:col-span-8 bg-white shadow-md rounded-2xl p-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 my-8">
            <div className="bg-gray-100 p-4 rounded-lg text-center">
              ...
              <p className="text-2xl font-bold">{summary.uploads}</p>
            </div>
            <div className="bg-gray-100 p-4 rounded-lg text-center">
              <h3 className="text-lg font-semibold flex items-center justify-center gap-2">
      ...
      <div className="mt-6 bg-white shadow-md rounded-2xl p-6 relative">
        ...
        {pageItems.length ? (
          ...
          !isLoading && <p className="text-gray-700">No items match your filters.</p>
        )}
      </div>
    </div>
  );
